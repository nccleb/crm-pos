...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_product'])) {
        ...
        if ($r) posLog($conn, $agent_id, $agent_name, 'product_added',
            "Product added: $nomp — LL " . number_format($price) . " — $category", mysqli_insert_id($conn), 'product');
    }
    ...
    if (isset($_POST['edit_product'])) {
        ...
        if ($err) {
            ...
            posLog($conn, $agent_id, $agent_name, 'product_edited',
                "Product updated: $nomp — LL " . number_format($price) . " — active: $active", $id, 'product');
    ...
    if (isset($_POST['delete_product'])) {
        $id = (int)$_POST['codep'];
        $prow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT nomp FROM produit WHERE codep=$id"));
        mysqli_query($conn, "UPDATE produit SET active=0 WHERE codep=$id");
        posLog($conn, $agent_id, $agent_name, 'product_deactivated',
            "Product deactivated: " . ($prow['nomp'] ?? "#$id"), $id, 'product');
